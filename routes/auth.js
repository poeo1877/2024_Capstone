...
router.get('/register', (req, res) => {
    res.render('register', { error: null, showMenu: false, showHeaderFooter: false });
});

// 로그인 페이지 렌더링
router.get('/login', (req, res) => {
    res.render('login', { error: null, showMenu: false, showHeaderFooter: false });
});
...
router.get('/tracking-pixel.png', (req, res) => {
    ...
    const buffer = Buffer.from(
        'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/wcAAgAB/eqrcBIAAAAASUVORK5CYII=',
        'base64'
    );
    res.send(buffer);
});
...
router.post('/reset-password/:token', async (req, res) => {
    ...
    try {
        ...
        if (isSamePassword) {
            return res.render('reset-password', { token: req.params.token, error: 'New password cannot be the same as the old password.', showMenu: false, showHeaderFooter: false });
        }
...
router.get('/logout', (req, res) => {
    req.session.destroy((err) => {
        if (err) {
            ...
            return res.status(500).render('login', { error: 'Failed to logout.' });
        }
        res.redirect('/login');
    });
});
