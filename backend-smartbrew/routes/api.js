	...
	try {
		...
		if (temperatureLimits.length > 0) {
			...
			if (measurement.in_temperature > limit.upper_limit) {
				const exceedAmount = measurement.in_temperature - limit.upper_limit;
				alerts.push(`온도가 상한값을 ${exceedAmount.toFixed(2)}도 초과했습니다!`);
			} else if (measurement.in_temperature < limit.lower_limit) {
				const belowAmount = limit.lower_limit - measurement.in_temperature;
				alerts.push(`온도가 하한값보다 ${belowAmount.toFixed(2)}도 낮습니다!`);
			}
		}
		...
		if (co2Limits.length > 0) {
			...
			if (measurement.co2_concentration > limit.upper_limit) {
				const exceedAmount = measurement.co2_concentration - limit.upper_limit;
				alerts.push(`CO2 농도가 상한값을 ${exceedAmount.toFixed(2)}ppm 초과했습니다!`);
			} else if (measurement.co2_concentration < limit.lower_limit) {
				const belowAmount = limit.lower_limit - measurement.co2_concentration;
				alerts.push(`CO2 농도가 하한값보다 ${belowAmount.toFixed(2)}ppm 낮습니다!`);
			}
		}
		...
		if (pHLimits.length > 0) {
			...
			if (measurement.ph > limit.upper_limit) {
				...
				alerts.push(`pH가 상한값을 ${exceedAmount.toFixed(2)}pH 초과했습니다!`);
			} else if (measurement.ph < limit.lower_limit) {
				const belowAmount = limit.lower_limit - measurement.ph;
				alerts.push(`pH가 하한값보다 ${belowAmount.toFixed(2)}pH 낮습니다!`);
			}
		}
...
router.post('/sensor/stop', async (req, res) => {
	try {
		...
		res.status(200).json({
			message: 'Sensor stopped successfully and fermenter status updated.',
		});
	} catch (error) {
		console.error(error);
...
router.post('/sensor/measurement', async (req, res) => {
	...
	try {
        ...
        */
		let { co2_concentration, brix, measured_time, out_temperature, in_temperature, ph, pressure_upper } = req.body;
...
router.get('/sensor/latest', async (req, res) => {
	try {
		...
		const phDifference = parseFloat((parseFloat(latestData.ph) - parseFloat(previousData.ph)).toFixed(3));
		const phPercentageDifference = parseFloat(((phDifference / parseFloat(previousData.ph)) * 100).toFixed(3));
...
router.get('/sensor/dashboard/latest', async (req, res) => {
	...
	// getLatestSensorDataByBatchId 함수 호출
	const latestTwoSensorData = await getLatestSensorDashboardByBatchId(batchId);
...
router.post('/material/receipt', async (req, res) => {
	...
	try {
		...
		await db.RawMaterial.update(
			{
				today_stock: db.sequelize.literal(`today_stock + ${parseFloat(quantity)}`),
	...
	conn.on('ready', () => {
		...
		conn.exec('sudo poweroff', (err, stream) => {
			if (err) {
				...
				return res.status(500).send('라즈베리파이 종료 중 오류가 발생했습니다.');
			}
