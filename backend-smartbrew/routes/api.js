	...
	try {
		...
		if (pressureLimits.length > 0) {
			...
			if (measurement.pressure_upper > limit.upper_limit) {
				const exceedAmount = measurement.pressure_upper - limit.upper_limit;
				alerts.push(`압력이 상한값을 ${exceedAmount.toFixed(2)}Pa 초과했습니다!`);
			} else if (measurement.pressure_lower < limit.lower_limit) {
				const belowAmount = limit.lower_limit - measurement.pressure_lower;
				alerts.push(`압력이 하한값보다 ${belowAmount.toFixed(2)}Pa 낮습니다!`);
			}
		}
